...
int main(int argc, char *argv[])
{
    ...
    //pp2.EnableAsciiAll (ascii.CreateFileStream ("tcp-bulk-send.tr"));
    pp2.EnablePcapAll ("ddosTraffic");
    //PcapHelper pcapHelper;
    //sink->TraceConnectWithoutContext("Rx", MakeCallback(&PacketReceived));
    //Ptr<PacketSink> sink = DynamicCast<PacketSink>(nodes.Get(2)->GetApplication(0));
    //Ptr<PcapFileWrapper> file = pcapHelper.CreateFile ("ddosTraffic.pcap", std::ios::out, PcapHelper::DLT_PPP);
    //sink->TraceConnectWithoutContext ("PhyRxDrop", MakeBoundCallback (&RxDrop, file));
    //AsciiTraceHelper asciiTraceHelper;
    //Ptr<Socket> TcpSink = Socket::CreateSocket (nodes.Get (0), TcpSocketFactory::GetTypeId ());
    //Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream ("ddos.cwnd");
    //TCPSink->TraceConnectWithoutContext ("CongestionWindow", MakeBoundCallback (&CwndChange, stream));
    /*
    Ptr<PacketSink> sink = DynamicCast<PacketSink>(nodes.Get(2)->GetApplication(0));
    PcapHelper pcapHelper;
    Ptr<PcapFileWrapper> file = pcapHelper.CreateFile ("doooos.pcap", std::ios::out, PcapHelper::DLT_RAW);
    sink->TraceConnectWithoutContext ("Rx", MakeBoundCallback (&RxDrop, file));
    
    PcapWriterHelper pcapWriter("output-ddos.pcap", std::ios::out, PcapWriter::DLT_RAW);
    pcapWriter.SetAttribute("TimestampPrecision", TimeValue(Time::MS));
    ApplicationContainer pcapApps;
    pcapApps.Add(UDPSinkApp);
    pcapWriter.EnablePcap(nodes.Get(2), "udp-traffic", true, true);
    */
    //Run the Simulation
    Simulator::Run();
    Simulator::Destroy();
